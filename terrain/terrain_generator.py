# Modified from https://github.com/unitreerobotics/unitree_mujoco/blob/main/terrain_tool/terrain_generator.py
...
OUTPUT_SCENE_PATH = "assets/" + ROBOT + "/scene_terrain.xml"
    ...
    def AddPerlinHeighField(
        ...
        for y in range(image_width):
            for x in range(image_width):
                # Perlin noise
                noise_value = noise.pnoise2(x / smoothness,
                                            y / smoothness,
        ...
        cv2.imwrite("../assets/" + ROBOT + "/" + output_hfield_image,
                    terrain_image)
...
if __name__ == "__main__":
    ...
    tg.AddPerlinHeighField(
        position=[0.0, 0.0, -0.04],  # Terrain center position
        euler=[0.0, 0.0, 0.0],  # Terrain orientation relative to the world
        size=[20, 5],  # Terrain length and width
        height_scale=0.1,  # Maximum terrain height
        negative_height=0.05,  # Negative height in the z-axis direction
        image_width=256,  # Terrain height map image pixel size
        image_height=256,
        smoothness=0.001,  # Noise smoothness
        perlin_octaves=20,  # Perlin noise parameters
        perlin_persistence=50,
        perlin_lacunarity=50,
        output_hfield_image="height_field.png"  # Output height map image name
    )
