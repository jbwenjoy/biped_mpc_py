...
def _clip_within_precision(number, low, high, precision=_TOL):
    ...
    if (number < low - precision).any() or (number > high + precision).any():
        raise ValueError(
            "Input {:.12f} not inside range [{:.12f}, {:.12f}] with precision {}".format(number, low, high, precision)
        )
...
def _rmat_to_euler_xyx(rmat):
    ...
    if rmat[0, 0] < 1.0:
        if rmat[0, 0] > -1.0:
            y = np.arccos(_clip_within_precision(rmat[0, 0], -1.0, 1.0))
...
def quat_conj(quat):
    ...
    return np.stack([quat[..., 0], -quat[..., 1], -quat[..., 2], -quat[..., 3]], axis=-1).astype(np.float64)
...
def quat_log(quat, tol=_TOL):
    ...
    v_new = (
        v
        / np.linalg.norm(v + tol, axis=-1, keepdims=True)
        * np.arccos(_clip_within_precision(a - tol, -1.0, 1.0, precision=2.0 * tol))
        / q_norm
    )
    return np.stack([np.log(q_norm[..., 0]), v_new[..., 0], v_new[..., 1], v_new[..., 2]], axis=-1)
...
def quat_to_axisangle(quat):
    ...
    """
    angle = 2 * np.arccos(_clip_within_precision(quat[0], -1.0, 1.0))
