...
class MPC:
    ...
    def initialize_parameters(self, Q, R):
        ...
        if Q is not None:
            if len(Q) != 12:
                raise ValueError("Q len must be 12 for [Theta, p, Omega, v]")
        ...
        if R is not None:
            if len(R) != 6:
                raise ValueError("R len must be 6 for [F_x, F_y, F_z, M_x, M_y, M_z]")
        ...
        self.kp = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 2]]) * 700  # Swing leg gains
        self.kd = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]]) * 3
        self.swingHeight = 0.1
        self.y_offset = 0.04
        self.rl_offsets = np.zeros(4, dtype=np.float32)  # RL foot offsets [lx, ly, rx, ry]
...
class Biped:
    def __init__(self):
        self.m = 12  # Mass
        self.I = np.array([[0.532, 0, 0], [0, 0.5420, 0], [0, 0, 0.0711]])  # Inertia
...
class BipedalLocomotionMPC:
    def __init__(
        ...
        **kwargs,
        ...
        self.decimation = int(self.ctrl_dt / self.sim_dt)  # Num of sim steps per ctrl step, 20
    ...
    def run_step(self, x_fb, q, qd, gait=None):
        ...
        if self.step_counter % self.decimation == 0:
            ...
            if controls is not None:
                ...
                if self.logging:
                    self.logger.warning("Using prev solution due to invalid QP result, not updating result.")
    ...
    def get_reference_foot_trajectory(self, x_fb, t, foot, contact):
        ...
        foot_des_x_1 = (
            x_fb[3] + x_fb[9] * 1 / 2 * self.mpc.h / 2 * self.mpc.dt + self.mpc.kv * (x_fb[3] - self.mpc.x_cmd[3])
        )
        foot_des_x_2 = (
            x_fb[3] + x_fb[9] * 1 / 2 * self.mpc.h * self.mpc.dt + self.mpc.kv * (x_fb[3] - self.mpc.x_cmd[3])
        )

        foot_des_y_1 = (
            x_fb[4] + x_fb[10] * 1 / 2 * self.mpc.h / 2 * self.mpc.dt + self.mpc.kv * (x_fb[4] - self.mpc.x_cmd[4])
        )
        foot_des_y_2 = (
            x_fb[4] + x_fb[10] * 1 / 2 * self.mpc.h * self.mpc.dt + self.mpc.kv * (x_fb[4] - self.mpc.x_cmd[4])
        )
        foot_des_z = 0
    ...
    def set_desired_acc(self, acc, x_fb):
        ...
        self.gravity_proj_vec = self.gravity_proj_vec = np.concatenate([acc_ang, acc_lin]) + np.array(
            [0, 0, 0, 0, 0, -self.biped.g]
        )
    ...
    def solve_mpc(self, x_fb, t, foot, contact):
            ...
            [
                ...
                np.hstack(
                    [
                        ...
                        np.zeros(3),
                        np.array([0, -self.biped.mu, -1]) @ R.T,
                        np.zeros(3),
                    ]
                ),
                np.hstack(
                ...
                np.hstack(
                    [
                        lh * np.array([0, 1, -self.biped.mu]) @ R.T,
                        np.zeros(3),
                        np.array([0, self.biped.mu, 1]) @ R.T,
                        np.zeros(3),
                    ]
        ...
        A = sparse.vstack([Aeq, A_mu, A_f, A_LFh]).tocsc()
        l = np.concatenate(
            [
                beq.flatten(),  # 等式约束下界
                -BIG_NUM * np.ones(A_mu.shape[0]),  # 摩擦锥约束下界
                -BIG_NUM * np.ones(A_f.shape[0]),  # 力饱和约束下界
                -BIG_NUM * np.ones(A_LFh.shape[0]),  # 足端线性约束下界
            ]
        )
        u = np.concatenate([beq.flatten(), b_mu.flatten(), b_f.flatten(), b_LF.flatten()])

        # Cost matrices
        L = np.kron(np.eye(self.mpc.h), np.diag(self.mpc.Q))
        K = np.kron(np.eye(self.mpc.h), np.diag(self.mpc.R))
        P = sparse.csc_matrix(2 * (Bqp.T @ L @ Bqp + K))
        q = 2 * Bqp.T @ L @ (Aqp @ x_0 - y)
    ...
    def get_leg_kinematics(q0, q1, q2, q3, q4, side):
        ...
        Jm[0, 0] = sin(q0) * (0.04 * sin(q2 + q3 + q4) + 0.22 * sin(q2 + q3) + 0.22 * sin(q2) + 0.0135) + cos(q0) * (
            0.015 * side
            + cos(q1) * (0.018 * side + 0.0025)
            - sin(q1) * (0.04 * cos(q2 + q3 + q4) + 0.22 * cos(q2 + q3) + 0.22 * cos(q2))
        )
        ...
        Jm[0, 1] = -sin(q0) * (
            sin(q1) * (0.018 * side + 0.0025)
            + cos(q1) * (0.04 * cos(q2 + q3 + q4) + 0.22 * cos(q2 + q3) + 0.22 * cos(q2))
        )
        Jm[1, 1] = cos(q0) * (
            sin(q1) * (0.018 * side + 0.0025)
            + cos(q1) * (0.04 * cos(q2 + q3 + q4) + 0.22 * cos(q2 + q3) + 0.22 * cos(q2))
        )
        Jm[2, 1] = sin(q1) * (0.04 * cos(q2 + q3 + q4) + 0.22 * cos(q2 + q3) + 0.22 * cos(q2)) - cos(q1) * (
            0.018 * side + 0.0025
        )
        Jm[3, 1] = cos(q0)
        Jm[4, 1] = sin(q0)
        ...
        Jm[0, 2] = sin(q0) * sin(q1) * (0.04 * sin(q2 + q3 + q4) + 0.22 * sin(q2 + q3) + 0.22 * sin(q2)) - cos(q0) * (
            0.04 * cos(q2 + q3 + q4) + 0.22 * cos(q2 + q3) + 0.22 * cos(q2)
        )

        Jm[1, 2] = -sin(q0) * (0.04 * cos(q2 + q3 + q4) + 0.22 * cos(q2 + q3) + 0.22 * cos(q2)) - cos(q0) * sin(q1) * (
            0.04 * sin(q2 + q3 + q4) + 0.22 * sin(q2 + q3) + 0.22 * sin(q2)
        )
        ...
        Jm[0, 3] = sin(q0) * sin(q1) * (0.04 * sin(q2 + q3 + q4) + 0.22 * sin(q2 + q3)) - cos(q0) * (
            0.04 * cos(q2 + q3 + q4) + 0.22 * cos(q2 + q3)
        )

        Jm[1, 3] = -sin(q0) * (0.04 * cos(q2 + q3 + q4) + 0.22 * cos(q2 + q3)) - cos(q0) * sin(q1) * (
            0.04 * sin(q2 + q3 + q4) + 0.22 * sin(q2 + q3)
        )
        ...
        Jm[0, 4] = 0.04 * sin(q2 + q3 + q4) * sin(q0) * sin(q1) - 0.04 * cos(q2 + q3 + q4) * cos(q0)

        Jm[1, 4] = -0.04 * cos(q2 + q3 + q4) * sin(q0) - 0.04 * sin(q2 + q3 + q4) * cos(q0) * sin(q1)
    ...
    def get_foot_pos_body(q0, q1, q2, q3, q4, side):
        ...
        pf[0] = (
            ...
            - (
                ...
                * np.sin(q4)
                * (
                    np.cos(q3) * (np.cos(q0) * np.cos(q2) - np.sin(q0) * np.sin(q1) * np.sin(q2))
                    - np.sin(q3) * (np.cos(q0) * np.sin(q2) + np.cos(q2) * np.sin(q0) * np.sin(q1))
                )
            )
            / 250
            ...
            - (
                ...
                * np.cos(q4)
                * (
                    np.cos(q3) * (np.cos(q0) * np.sin(q2) + np.cos(q2) * np.sin(q0) * np.sin(q1))
                    + np.sin(q3) * (np.cos(q0) * np.cos(q2) - np.sin(q0) * np.sin(q1) * np.sin(q2))
                )
            )
            / 250
        ...
        pf[1] = (
            ...
            - (
                ...
                * np.sin(q4)
                * (
                    np.cos(q3) * (np.cos(q2) * np.sin(q0) + np.cos(q0) * np.sin(q1) * np.sin(q2))
                    - np.sin(q3) * (np.sin(q0) * np.sin(q2) - np.cos(q0) * np.cos(q2) * np.sin(q1))
                )
            )
            / 250
            - (3 * np.sin(q0)) / 200
            ...
            - (
                ...
                * np.cos(q4)
                * (
                    np.cos(q3) * (np.sin(q0) * np.sin(q2) - np.cos(q0) * np.cos(q2) * np.sin(q1))
                    + np.sin(q3) * (np.cos(q2) * np.sin(q0) + np.cos(q0) * np.sin(q1) * np.sin(q2))
                )
            )
            / 250
    ...
    def low_level_control(self, x_fb, t, pf_w, q, qd, contact, u):
        ...
        for leg in range(2):
            ...
            # stance mapping
            u_b = -np.vstack([R.T @ u[3 * leg : 3 * leg + 3], R.T @ u[3 * leg + 6 : 3 * leg + 9]])
...
if __name__ == "__main__":
    ...
    mpc_params = {
        "Q": [
            ...
            1,
            1,
        ],  # [rx, ry, rz, x, y, z, wx, wy, wz, vx, vy, vz, gravity(ignored here)]
